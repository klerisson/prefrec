...
public class StratifiedMatrix implements Serializable {
	...
	public void addRestOfRows(Map<Integer, Integer[]> restOfRows) {
		for (Map.Entry<Integer, Integer[]> entry : restOfRows.entrySet()) {
			this.addRow(entry.getValue(), this.partitions.lastKey(), entry.getKey(), this.itemsId);
		}
	}
	
	public void addRestOfColumns(Map<Integer, Integer[]> restOfColumns) {
		for (Map.Entry<Integer, Integer[]> entry : restOfColumns.entrySet()) {
			this.addColumn(entry.getValue(), this.partitions.lastKey(), entry.getKey(), this.usersId);
		} 
	}
	...
	/**
	 * @param partitionsFromItem the partitionsFromItem to set
	 */
	public void setPartitionsFromItem(TreeMap<Integer, List<Integer[]>> partitionsFromItem) {
